...
namespace BaseUlt2
{
    class BaseUlt
    {
        ...
        {
            if (!_menu.Item("baseUlt1").GetValue<bool>())
                return;
        ...
        private static void HandleRecallShot(EnemyInfo playerInfo)
        {
                            ...
                            x.Mana >= x.Spellbook.GetSpell(SpellSlot.R).ManaCost)))) //use when fixed: champ.Spellbook.GetSpell(SpellSlot.R) = Ready or champ.Spellbook.GetSpell(SpellSlot.R).ManaCost)
            {
                ...
                var timeneeded = GetSpellTravelTime(champ, UltInfo[champ.ChampionName].Speed, UltInfo[champ.ChampionName].Delay, _enemySpawnPos) - (_menu.Item("extraDelay1").GetValue<Slider>().Value + 65);
            ...
            if (!shoot || _menu.Item("panicKey1").GetValue<KeyBind>().Active)
                return;
            ...
            if (time - playerInfo.LastSeen > 20000 && !_menu.Item("regardlessKey1").GetValue<KeyBind>().Active)
            {
        ...
        {
            if (!_menu.Item("showRecalls1").GetValue<bool>())
                return;
                ...
                x.Player.IsEnemy).OrderBy(x => x.RecallInfo.GetRecallEnd()))
            {
                ...
                Drawing.DrawText(Drawing.Width * 0.73f, Drawing.Height * 0.88f + (index * 15f), Color.Lime, playerInfo.RecallInfo.ToString());
            }
        }
